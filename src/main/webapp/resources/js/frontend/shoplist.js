$(function () {
    ...
    // 一页返回的最大条数
    var pageSize = 3;
    var listUrl = '/schooloto/frontend/listshops';
    var searchDivUrl = '/schooloto/frontend/listshopspageinfo';
    // 页码
    var pageNum = 1;
            ...
            if (data.success) {
                ...
                data.shopList.map(function (item, index) {
                    html += '' +
                        ...
                        + item.shopId + '">' + '<div class="card-header">' + item.shopName + '</div>'
                        ...
                        + '</div>'
                        + '</div>'
                        + '<div class="card-footer">' + '<p class="color-gray">' + new Date(item.lastEditTime).Format("yyyy-MM-dd") + '更新</p>' + '<span>点击查看</span>' + '</div>'
                        + '</div>';
                });
    ...
    $('.shop-list').on('click', 'card', function (e) {
        ...
        window.location.href = '/schooloto/frontend/shopdetail?shopId=' + shopId;
    });
